...
exports.transformFile = function(filename) {
    ...
    var n = narc.parser.parse(source)
    n.children = transform_statements(n.children)
...
var transformations = {
    ...
    'var': function(statement, statements) {
        for (var i in statement.children) {
            ...
            util.debug('var initializer checking is_streamlined_function_definition ' + pp(statement))

            if (is_streamlined_function_definition(expression)) {
                util.debug('is_streamlined_function_definition = true')
                transform_function(expression)
            } else {
                util.debug('is_streamlined_function_definition = false')
            }
        }
        return [statement].concat(transform_statements(statements))
    },
    
    'function': function(statement, statements) {
