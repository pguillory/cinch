...
function transform_statement(statement, statements) {
    ...
    //util.debug('ignoring statement: ' + pp(statement))
...
/*
        if (statement.tryBlock) {
            statement.tryBlock.children = transform_statements(statement.tryBlock.children)
        }

        for (var i in statement.catchClauses) {
            var block = statement.catchClauses[i].block
            block.children = transform_statements(block.children)
        }
*/

/*
        console.log(pp(statement))
        dump(statement)
        process.exit()
*/
        ...
        var result = [
            {
                type: RETURN,
                value: snippets.call('try_block', [
                    snippets.anonymous_function(['err'], [
