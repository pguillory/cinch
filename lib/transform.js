...
/*
function transform_subfunction(func) {
    if (ends_with_underscore(func.name)) {
        console.log('transforming subfunction ' + func.name)

        func.name = strip_underscore(func.name)
        func.params.push('block_callback')
...
var transformations = {
    'throw': function(statement, statements) {
        ...
        return snippets.return_callback(statement.exception)
    ...
    'try': function(statement, statements) {
        ...
        var func = snippets.function(
            ...
            transform_statements(
                ...
                .concat([
                    snippets.call(finally_block_ident, [])
                ])
            )
        )
        result.push(func)
