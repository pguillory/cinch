...
var transformations = {
    ...
    'try': function(statement, statements) {
        ...
        if (statement.catchClauses.length > 0) {
            result.push(snippets.return_try_block(try_block_ident.value, [
                snippets.if(snippets.err(), [
                    snippets.call(catch_block_ident.value, [
                        snippets.err(),
                        snippets.function_err([
                            snippets.call(finally_block_ident.value, [
                                snippets.function_err([
                                    snippets.call(rest_block_ident.value, [
                                        snippets.callback()
                                    ])
                                ])
                            ])
        ...
        var func = snippets.function(try_block_ident.value + '_', [], statement.tryBlock.children)
        transform_function(func)
        result.push(func)
        
        for (var i in statement.catchClauses) {
            var clause = statement.catchClauses[i]
            var func = snippets.function(catch_block_ident.value + '_', [clause.varName], clause.block.children)
            transform_function(func)
            result.push(func)
        }

        var func = snippets.function(finally_block_ident.value + '_', [], statement.finallyBlock.children)
        transform_function(func)
        result.push(func)

        var func = snippets.function(rest_block_ident.value + '_', [], statements)
        transform_function(func)
        result.push(func)
