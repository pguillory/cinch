...
function transform_children(children) {
    for (var i in children) {
        ...
        switch (n.type) {
            case THROW:
                children.splice(parseInt(i), 1, snippets.return_callback(n.exception))
                break
            case IDENTIFIER:
                break
            case SEMICOLON:
...
*/
                break
            case RETURN:
                if (n.value && is_streamlined_function_call(n.value)) {
                    n.value.children[0].value = strip_underscore(n.value.children[0].value)
                    n.value.children[1].children.push({
                        type: IDENTIFIER,
                        value: 'callback',
                    })
                } else if (n.value && contains_streamlined_function_call(n.value.children)) {
                    ...
                    n.value = found.n
                } else if (n.value && contains_regular_function_call(n.value.children)) {
                    ...
                    if (n.value) {
                        n = snippets.return_callback_value(n.value)
                    } else {
                        n = snippets.return_callback_null()
                    }
                    children.splice(parseInt(i), 1, n)
