...
var transformations = {
    ...
    'break': function(statement, statements) {
        util.debug('break: ' + pp(statement))
        util.debug('target type: ' + tokens[statement.target.type])
        util.debug('target rest_ident: ' + statement.target.rest_ident.value)
...
/*
        for (var i in statement.body.children) {
            if (statement.body.children[i].type === BREAK) {
                statement.body.children.splice(i, statement.body.children.length, snippets.call(rest_ident, []))
            }
        }
*/
