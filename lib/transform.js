...
function transform_function(func) {
    ...
    if (ends_with_underscore(func.name)) {
        ...
        transform_statements(func.body.children)
    }
...
function transform_statements(statements) {
    for (var i in statements) {
        ...
        // console.log('child ' + tokens[statement.type])

        switch (statement.type) {
            case THROW:
                statements.splice(parseInt(i), 1, snippets.return_callback(statement.exception))
                break
            case IDENTIFIER:
                break
            case SEMICOLON:
                ...
                break
            case RETURN:
                if (statement.value && is_streamlined_function_call(statement.value)) {
                ...
                } else if (statement.value && contains_streamlined_function_call(statement.value.children)) {
                    ...
                    statement.value = found.n
                } else if (statement.value && contains_regular_function_call(statement.value.children)) {
                    console.log('return contains_regular_function_call')
                    var ident = get_ident()
                    
                    var found = find_regular_function_call(statement.value.children)
                    statements.splice(parseInt(i), 0, snippets.try_catch_return_callback_err({
                        type: SEMICOLON,
                        expression: {
                            type: ASSIGN,
                            children: [
                    ...
                    if (statement.value) {
                        statement = snippets.return_callback_value(statement.value)
                    } else {
                        statement = snippets.return_callback_null()
                    }
                    statements.splice(parseInt(i), 1, statement)
/*                    statement.value = snippets.callback_null(statement.value)*/
                }
                break
            case FUNCTION:
            ...
            case TRY:
                ...
                break
            default:
                util.debug('unknown type: ' + tokens[statement.type])
                util.debug(pp(statement))
                transform_statements(statement.children)
                break
        }
    }
}
